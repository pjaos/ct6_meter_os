...
import com.example.wifidevsetup.BuildConfig;
 ...
 */
public class BTLESerialSocket extends BluetoothGattCallback {
    ...
    public BTLESerialSocket(Context context, BluetoothDevice device) {
    ...
    public void disconnect() {
        StaticResources.AppLibHelper.debug("disconnect");
        ...
        if(delegate != null)
            delegate.disconnect();
        if (gatt != null) {
            StaticResources.AppLibHelper.debug("gatt.disconnect");
            gatt.disconnect();
            StaticResources.AppLibHelper.debug("gatt.close");
            try {
                gatt.close();
    ...
    public void connect(SerialListener listener) throws IOException {
        ...
        context.registerReceiver(disconnectBroadcastReceiver, new IntentFilter(BTLESerialSocket.INTENT_ACTION_DISCONNECT));
        StaticResources.AppLibHelper.debug("connect "+device);
        context.registerReceiver(pairingBroadcastReceiver, pairingIntentFilter);
        if (Build.VERSION.SDK_INT < 23) {
            StaticResources.AppLibHelper.debug("connectGatt");
            gatt = device.connectGatt(context, false, this);
        } else {
            StaticResources.AppLibHelper.debug("connectGatt,LE");
     ...
     * @param listener
     */
    public void setListner(SerialListener listener) {
        this.listener = listener;
    }
    ...
    private void onPairingBroadcastReceive(Context context, Intent intent) {
        ...
        switch (intent.getAction()) {
            case BluetoothDevice.ACTION_PAIRING_REQUEST:
                ...
                StaticResources.AppLibHelper.debug("pairing request " + pairingVariant);
                onSerialConnectError(new IOException("PAIRING requested\n-> Perform pairing steps. Connect again"));
            ...
            case BluetoothDevice.ACTION_BOND_STATE_CHANGED:
                ...
                StaticResources.AppLibHelper.debug("bond state " + previousBondState + "->" + bondState);
                break;
            default:
                StaticResources.AppLibHelper.debug("unknown broadcast " + intent.getAction());
                break;
        }
    }
    ...
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        ...
        if (newState == BluetoothProfile.STATE_CONNECTED) {
            StaticResources.AppLibHelper.debug("connect status "+status+", discoverServices");
            ...
            StaticResources.AppLibHelper.debug("unknown connect state "+newState+" "+status);
        }
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        StaticResources.AppLibHelper.debug("servicesDiscovered, status " + status);
        if (canceled)
            return;
    ...
    private void connectCharacteristics1(BluetoothGatt gatt) {
        ...
        if(delegate==null || readCharacteristic==null || writeCharacteristic==null) {
            for (BluetoothGattService gattService : gatt.getServices()) {
                StaticResources.AppLibHelper.debug("service "+gattService.getUuid());
                for(BluetoothGattCharacteristic characteristic : gattService.getCharacteristics())
                    StaticResources.AppLibHelper.debug("characteristic "+characteristic.getUuid());
            }
    ...
    private void connectCharacteristics2(BluetoothGatt gatt) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            StaticResources.AppLibHelper.debug("request max MTU");
    ...
    public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
        StaticResources.AppLibHelper.debug("mtu size "+mtu+", status="+status);
        if(status ==  BluetoothGatt.GATT_SUCCESS) {
            payloadSize = mtu - 3;
            StaticResources.AppLibHelper.debug("payload size "+payloadSize);
        }
        connectCharacteristics3(gatt);
    }
    ...
    private void connectCharacteristics3(BluetoothGatt gatt) {
        ...
        if((readProperties & BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0) {
            StaticResources.AppLibHelper.debug("enable read indication");
            ...
            StaticResources.AppLibHelper.debug("enable read notification");
        ...
        StaticResources.AppLibHelper.debug("writing read characteristic descriptor");
    ...
    public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        ...
        if(descriptor.getCharacteristic() == readCharacteristic) {
            StaticResources.AppLibHelper.debug("writing read characteristic descriptor finished, status="+status);
            if (status != BluetoothGatt.GATT_SUCCESS) {
                ...
                onSerialConnect();
                StaticResources.AppLibHelper.debug("connected");
            }
        }
    }
    ...
    public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        ...
        if(characteristic == readCharacteristic) { // NOPMD - test object identity
            ...
            onSerialRead(data);
            StaticResources.AppLibHelper.debug("read, len="+data.length);
        }
    }
    ...
    public void write(byte[] data) throws IOException {
        ...
        synchronized (writeBuffer) {
            ...
            if(!writePending && writeBuffer.isEmpty() && delegate.canWrite()) {
                ...
                StaticResources.AppLibHelper.debug("write queued, len="+data0.length);
                data0 = null;
            }
            if(data.length > payloadSize) {
                for(int i=1; i<(data.length+payloadSize-1)/payloadSize; i++) {
                    ...
                    StaticResources.AppLibHelper.debug("write queued, len="+(to-from));
                }
            }
        }
        if(data0 != null) {
            ...
            if (!gatt.writeCharacteristic(writeCharacteristic)) {
                ...
                StaticResources.AppLibHelper.debug("write started, len="+data0.length);
            }
        }
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if(characteristic == writeCharacteristic) { // NOPMD - test object identity
            StaticResources.AppLibHelper.debug("write finished, status="+status);
            writeNext();
        }
    }
    ...
    private void writeNext() {
        ...
        if(data != null) {
            ...
            if (!gatt.writeCharacteristic(writeCharacteristic)) {
                ...
                StaticResources.AppLibHelper.debug("write started, len="+data.length);
            }
        }
    }
    ...
    private class Cc245XDelegate extends DeviceDelegate {
        ...
        boolean connectCharacteristics(BluetoothGattService gattService) {
            StaticResources.AppLibHelper.debug("service cc254x uart");
    ...
    private class Rn4870Delegate extends DeviceDelegate {
        @Override
        boolean connectCharacteristics(BluetoothGattService gattService) {
            StaticResources.AppLibHelper.debug("service rn4870 uart");
    ...
    private class NrfDelegate extends DeviceDelegate {
        @Override
        boolean connectCharacteristics(BluetoothGattService gattService) {
            StaticResources.AppLibHelper.debug("service nrf uart");
            ...
            if (rw2 != null && rw3 != null) {
                ...
                StaticResources.AppLibHelper.debug("characteristic properties " + rw2prop + "/" + rw3prop);
    ...
    private class TelitDelegate extends DeviceDelegate {
        ...
        @Override
        boolean connectCharacteristics(BluetoothGattService gattService) {
            StaticResources.AppLibHelper.debug("service telit tio 2.0");
            readCredits = 0;
            writeCredits = 0;
            ...
            StaticResources.AppLibHelper.debug("writing read credits characteristic descriptor");
        ...
        void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            if(descriptor.getCharacteristic() == readCreditsCharacteristic) {
                StaticResources.AppLibHelper.debug("writing read credits characteristic descriptor finished, status=" + status);
                if (status != BluetoothGatt.GATT_SUCCESS) {
        ...
        void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            if(characteristic == readCreditsCharacteristic) { // NOPMD - test object identity
                ...
                StaticResources.AppLibHelper.debug("got write credits +"+newCredits+" ="+writeCredits);

                if (!writePending && !writeBuffer.isEmpty()) {
                    StaticResources.AppLibHelper.debug("resume blocked write");
                    writeNext();
                }
            }
            if(characteristic == readCharacteristic) { // NOPMD - test object identity
                grantReadCredits();
                StaticResources.AppLibHelper.debug("read, credits=" + readCredits);
            }
        }
        ...
        void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            if(characteristic == writeCharacteristic) { // NOPMD - test object identity
                ...
                StaticResources.AppLibHelper.debug("write finished, credits=" + writeCredits);
            }
            if(characteristic == writeCreditsCharacteristic) { // NOPMD - test object identity
                StaticResources.AppLibHelper.debug("write credits finished, status="+status);
            }
        }
        ...
        boolean canWrite() {
            if(writeCredits > 0)
                return true;
            StaticResources.AppLibHelper.debug("no write credits");
            return false;
        }
        ...
        private void grantReadCredits() {
            ...
            if(readCredits <= minReadCredits) {
                ...
                StaticResources.AppLibHelper.debug("grant read credits +"+newCredits+" ="+readCredits);
